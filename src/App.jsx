...
const buscar=async()=>{if(!busqueda.trim()){setError("Escribe un Pokémon");return}try{setCargando(true);setError("");setPokemon(null);
const r=await fetch("https://pokeapi.co/api/v2/pokemon/"+busqueda.trim().toLowerCase());if(!r.ok)throw new Error("Pokémon no encontrado");
const d=await r.json();const ataque=d.stats.find(s=>s.stat.name==="defense")?.base_stat??0;
const total=d.stats.reduce((a,s)=>a+s.base_stat,0);
const altura=d.height/10;
const clasificacion=total<300?"Pokémon poderoso":"Pokémon normal";setPokemon({nombre:d.name,imagen:d.sprites?.front_default,altura,peso:d.weight/10,ataque,total,clasificacion,tipos:d.types.map(t=>t.type.name)})}catch(e){setError(e.message)}finally{setCargando(false)}};
